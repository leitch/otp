...
- **Exclusions:** Ignore binary files, massive datasets (>1MB), and temporary log files to focus on logic and configuration.
...
- **Access Control:** Look for overly permissive file permissions or insecure [CORS/Auth configurations](https://owasp.org).

### 3. Compliance Frameworks
- **SOC 2 (Trust Services Criteria):** 
    - Identify lack of [Role-Based Access Control (RBAC)](https://complydog.com) in skill execution.
    - Look for missing audit trails or logging for critical system or tool-calling actions.
- **GDPR (Privacy by Design):** 
    - **Data Minimization:** Flag instances where [excessive personal data (PII)](https://gdpr-info.eu) is collected or stored.
    - **Anonymization:** Detect where pseudonymization should be applied before data is sent to an external LLM provider.
...
2. **Detailed Findings:** For each issue found, list:
   ...
   - **Compliance Map:** State which [SOC 2 criteria or GDPR article](https://linfordco.com) this finding relates to.
